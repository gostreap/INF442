...
void dendrogram::merge(edge *e) {
	...
	int representative1 = find(e->get_p1());
	int representative2 = find(e->get_p2());
	if (representative1 == representative2) {
		...
		left[representative2] = down[representative1];
		height[representative2] = e->get_length() / 2;
		down[representative1] = representative2;
	} else if (rank[representative1] < rank[representative2]) {
		// representative = representative2;
		parent[representative1] = representative2;
		left[representative1] = down[representative2];
		height[representative1] = e->get_length() / 2;
		down[representative2] = representative1;
	} else if (representative1 < representative2) {
		// representative = representative2;
		parent[representative1] = representative2;
		left[representative1] = down[representative2];
		height[representative1] = e->get_length() / 2;
...
double dendrogram::get_dendro_height() { return height[down[find(0)]]; }
...
long dendrogram::get_n() { return g->get_num_nodes(); }
...
int dendrogram::get_count_sign_heights() { return count_sign_heights; }
...
int dendrogram::get_total_clusters() { return total_clusters; }

double dendrogram::get_cut_height() { return cut_height; }

/****** Methods for testing ******/

void dendrogram::print_node(int i) {
	cout << "Node " << i << "(parent =  " << parent[i] << ", down = " << down[i] << ", left = " << left[i]
		 << ", rank = " << rank[i] << ", height = " << height[i] << ", cluster = " << clusters[i] << ")";
}

void dendrogram::print_dendrogram() {
	cout << "node\tparent\trank\tleft\tdown\theight\tcluster" << endl;
	for (int i = 0; i < get_n(); i++)
		cout << i << "\t" << parent[i] << "\t" << rank[i] << "\t" << left[i] << "\t" << down[i] << "\t" << height[i]
			 << "\t" << clusters[i] << "\t" << endl;
}

void dendrogram::print_clusters() {
	int print_after[g->get_num_nodes()];
	for (long i = 0; i < g->get_num_nodes(); i++) {
		print_after[i] = -1;
	}
	...
	for (int i = 0; i < g->get_num_nodes(); i++) {
		...
		if (clusters[i] == i) {
			...
			cout << "Cluster \"" << clusters[i] << "\" (node: " << i << ";"
				 << " height: " << get_cluster_height(i) << ")" << endl;
...
void dendrogram::iterate_sign_heights() {
	...
	cout << "Setting clusters at height " << cut << "..." << endl;
	clear_clusters();
	set_clusters(cut);
	cout << "\t" << count_ns_clusters() << " non-singleton cluster" << (count_ns_clusters() > 1 ? "s" : "") << " below "
		 << cut << endl;
